...
interface ReportViewProps {
  ...
  designImage: string;
  designFileName?: string; // Add optional filename prop
    ...
    <div className="w-full h-full bg-transparent overflow-y-auto animate-fade-in custom-scrollbar relative z-10">
      ...
      <div className="max-w-4xl mx-auto py-10 px-6">
        ...
        <div className="bg-bg-card/60 backdrop-blur-xl border border-white/10 rounded-3xl p-8 mb-10 flex items-center justify-between shadow-glow relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-lg">
                <h3 className="text-2xl font-bold text-text-main mb-2">
                    {designFileName ? `${designFileName} 页面分析报告` : t('report.summaryTitle')}
                </h3>
            ...
            {/* Score Display - Text Only */}
            <div className="flex flex-col items-center justify-center shrink-0 ml-8 mt-4">
                 <div className="flex flex-col items-center justify-center p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm shadow-lg">
                    <span className="text-xs text-text-muted font-bold uppercase tracking-wider mb-1">{t('analysis.scoreTitle')}</span>
                    <div className="flex items-baseline gap-1">
                        <span className={`text-6xl font-black ${scoreColor} tracking-tighter`}>{result.score}</span>
                        <span className="text-xl text-text-muted font-bold opacity-60">/100</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="mt-12 pt-8 border-t border-white/5 text-center flex flex-col gap-2">
            <p className="text-xs text-text-muted/60 max-w-2xl mx-auto">
                {t('report.disclaimer')}
            </p>
